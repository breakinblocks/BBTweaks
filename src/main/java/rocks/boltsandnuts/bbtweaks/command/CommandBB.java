...
public class CommandBB extends CommandBase {
    ...
    public static void giveBreakBit(ICommandSender command) {
        ...
        if (time - timeNoSee < cooldown) {
            ...
            player.addChatMessage(new ChatComponentTranslation("You aren't eligible for another Breakbit yet."));
        ...
        if (!player.inventory.addItemStackToInventory(BB)) {
            player.addChatMessage(new ChatComponentTranslation("Not enough inventory Space."));
        } else {
            player.addChatMessage(new ChatComponentTranslation("Granted your Daily Invar BreakBit!"));
